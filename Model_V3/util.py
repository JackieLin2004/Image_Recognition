...
def plot_result(x, smooth_y1, smooth_y2, smooth_y3, smooth_y4, smooth_y5, title, y_label, path):
    """绘制原始数据与平滑数据的对比图"""
    plt.figure(figsize=(10, 6))
    # 原始图
    # plt.plot(x, y, label='Original', alpha=0.5)
    # 平滑处理后的图
    plt.plot(x[len(x) - len(smooth_y1):] if len(smooth_y1) < len(x) else x, smooth_y1,
             label='5:5', color='green', linewidth=2)
    plt.plot(x[len(x) - len(smooth_y2):] if len(smooth_y2) < len(x) else x, smooth_y2,
             label='6:4', color='red', linewidth=2)
    plt.plot(x[len(x) - len(smooth_y3):] if len(smooth_y3) < len(x) else x, smooth_y3,
             label='7:3', color='yellow', linewidth=2)
    plt.plot(x[len(x) - len(smooth_y4):] if len(smooth_y4) < len(x) else x, smooth_y4,
             label='8:2', color='blue', linewidth=2)
    plt.plot(x[len(x) - len(smooth_y5):] if len(smooth_y5) < len(x) else x, smooth_y5,
             label='9:1', color='orange', linewidth=2)
    plt.title(title)
    plt.xlabel('Epoch')
    plt.ylabel(y_label)
    plt.legend()
